	...
	director = func(ctx context.Context, fullMethodName string) (context.Context, *grpc.ClientConn, error) {
		...
		if ok {
			...
			if val, exists := md[":authority"]; exists && val[0] == "staging.api.example.com" {
				...
				conn, err := grpc.DialContext(ctx, "api-service.staging.svc.local", grpc.WithDefaultCallOptions(grpc.CallCustomCodec(proxy.Codec())))
				return outCtx, conn, err
			} else if val, exists := md[":authority"]; exists && val[0] == "api.example.com" {
				conn, err := grpc.DialContext(ctx, "api-service.prod.svc.local", grpc.WithDefaultCallOptions(grpc.CallCustomCodec(proxy.Codec())))
				return outCtx, conn, err
			}
		}
